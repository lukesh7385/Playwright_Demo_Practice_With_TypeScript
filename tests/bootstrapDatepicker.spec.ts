...
test("Booking.com Date Picker Test - Check-in and Check-out", async ({ page }) => {
    ...
    // Check on the date picker field to open calendar
    await page.getByTestId("searchbox-dates-container").click();

    //============== Check-in Date Selector ==============
    let checkinYear: string = "2026";
    let checkinMonth: string = "June";
    let checkinDay: string = "29";

    // Navigte through the calender to find the desired check-in month and year
    while(true){
    ...
    while(true){
        ...
        if(currentMonth === checkoutMonth && currentYear === currentYear){
            break;
        }
        else{
            await page.locator("button[aria-label='Next month']").click();
    ...
    await page.waitForTimeout(5000); // just to visually abserve the result during test run(optional)
