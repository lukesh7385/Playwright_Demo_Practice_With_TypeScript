...
test("Autowaiting and forcing", async ({ page }) => {
    ...
    await page.locator(".search-box-text").fill("Laptop", {force:true}); // Search box Force action (it will not so actionability checks)
    await page.locator(".button-1.search-box-button").click({force:true}); // clicking on the search button - Force action
