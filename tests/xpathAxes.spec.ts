...
test("Xpath Axes demo", async ({ page }) => {
    ...
    // 1. self axis - select the <td> that contains "Germany"
    ...
    // 6. following axis - Get the <td> comes after "Germany" in document order
    ...
    const precedingCall: Locator = page.locator("//td[text() = 'Germany']/preceding::td[1]");
    await expect(precedingCall).toHaveText("Maria Anders");
